...
public class UISystemFacade : IUISystemFacade
{
    ...
    public T RunVisitor<T>(T visitor) where T : IUIComponentVisitor
    {
	if (_rootTree == null)
	    throw new InvalidOperationException("Root tree is null");
	
	if (_rootTree is IAcceptable acceptable)
    ...
    public void RenderAllToContext(IRenderingContext ctx)
    {
        ...
        try
        {
            ...
            _telemetry.LogOperation("Facade", "RenderAll", duration, null);
        }
        finally
        {
            _lock.ExitReadLock();
