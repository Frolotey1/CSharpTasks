...
public class PanelComponent : UIComponentBase, IContainerComponent
{
    ...
    public void ApplyLayout(LayoutContext context)
    {
        if (_layoutStrategy == null) return;
        var bounds = _layoutStrategy.CalculateBounds(this, context);
        foreach (var kvp in bounds)
        {
            kvp.Key.SetPosition(new Point(kvp.Value.X, kvp.Value.Y));
        }
    }
    ...
    public override void Accept(IUIComponentVisitor visitor)
    {
        ...
        foreach (var child in _children)
        {
            if (child is IAcceptable acceptable)
